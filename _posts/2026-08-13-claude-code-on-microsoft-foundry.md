...
![Seven verification checks passing: Azure CLI sign-in, Entra ID token, Claude deployments found, Messages API responds, CLI installed, provider = foundry, and an end-to-end Claude Code turn on Foundry]({{ img | append: '/verify.png' | relative_url }})
...
![The Claude Code panel in VS Code answering a question about a file after calling its Glob and Read tools, with no sign-in prompt]({{ img | append: '/answer.png' | relative_url }})

![Animated walkthrough: the Claude Code panel opens without a login prompt, then answers a question about the codebase using its file tools]({{ img | append: '/extension.gif' | relative_url }})
